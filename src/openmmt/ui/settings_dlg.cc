...
LRESULT DlgSettings::OnInitDialog(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL &bHandled)
{
  ...
  m_btnSetLooseFocus.SetCheck(g_bOptions_ShowTaskbarWhenFullAppLostFocus ? 1 : 0);
...
LRESULT DlgSettings::OnApply(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL &bHandled)
{
  ...
  if ((m_btnSetLooseFocus.GetCheck() == 0) && (g_bOptions_ShowTaskbarWhenFullAppLostFocus)) {
    g_bOptions_ShowTaskbarWhenFullAppLostFocus = FALSE;
  } else if ((m_btnSetLooseFocus.GetCheck()) && (!g_bOptions_ShowTaskbarWhenFullAppLostFocus)) {
    g_bOptions_ShowTaskbarWhenFullAppLostFocus = TRUE;
  }
